...
namespace Unity.AI.Assistant.UI.Editor.Scripts.Components.ChatElements
{
    class AssistantWindowUiContainer : IToolUiContainer, IDisposable
    {
        ...
        void TryRestoreFromSessionState()
        {
            ...
            var (items, planPath) = AssistantUISessionState.instance.GetTodoState(lastActiveId);
            if (items == null || items.Count == 0) return;

            m_TodoConversationId = lastActiveId;
            RestoreTodoPanel(items, planPath);
        }
        ...
        void OnConversationReload(AssistantConversationId conversationId)
        {
            ...
            var (items, planPath) = AssistantUISessionState.instance.GetTodoState(id);
        ...
        void OnTodoListUpdated(List<TodoItem> items, string planPath, string conversationId)
        {
            ...
            AssistantUISessionState.instance.SetTodoState(conversationId, items, planPath);
        ...
        void RestoreTodoPanel(List<TodoItem> items, string planPath)
        {
            if (m_TodoInteraction == null)
            {
                m_TodoInteraction = new TodoProgressInteractionElement(planPath);
