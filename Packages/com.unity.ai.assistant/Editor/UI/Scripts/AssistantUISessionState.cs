...
namespace Unity.AI.Assistant.UI.Editor.Scripts
{
    internal class AssistantUISessionState : ScriptableSingleton<AssistantUISessionState>
    {
        ...
        public void SetTodoState(string conversationId, List<TodoItem> items, string planPath)
        {
            ...
            {
                cache.entries.Add(new TodoStateEntry { conversationId = conversationId, items = items ?? new List<TodoItem>(), planPath = planPath });
            }
            SaveTodoStateCache(cache);
        }

        public (List<TodoItem> items, string planPath) GetTodoState(string conversationId)
        {
            var cache = LoadTodoStateCache();
            var entry = cache.entries.Find(e => e.conversationId == conversationId);
            if (entry == null || entry.items == null || entry.items.Count == 0)
                return (null, null);
            return (entry.items, entry.planPath);
        }
