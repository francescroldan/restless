...
namespace Unity.AI.MCP.Editor
{
    ...
    class Bridge : IDisposable
    {
        ...
        async Task ValidateAndApproveAsync(IConnectionTransport transport, CancellationToken token, bool isBatchMode)
        {
            try
            {
                ...
                if (!isGateway)
                {
                    ...
                    int currentDirect = TransportStore.CountDirectConnections();
                    bool overCapacity = maxDirect >= 0 && currentDirect >= maxDirect;
        ...
        {
            return TransportStore.CountDirectConnections();
        }
