    ...
    static class Quote
    {
        ...
        static async Task QuoteMaterialsAsync(QuoteMaterialsData arg, AsyncThunkApi<bool> api)
        {
            ...
            try
            {
                ...
                bool EnsureReferenceImageExists()
                {
                    var promptImageAsset = api.State.SelectPromptImageReferenceAsset(asset);
                    if (refinementMode == RefinementMode.Pbr && promptImageAsset.IsValid() && promptImageAsset.Exists())
                        return true;
                    if (api.State.HasReferenceAssetWithFallback(asset))
                        return true;

                    var messages = new[] { "No reference image available. Please assign a base texture to the material or select a generated result." };
                    api.Dispatch(GenerationActions.setGenerationValidationResult,
                        new(arg.asset, new(false, BackendServiceConstants.ErrorTypes.Unknown, 0, messages.Select(m => new GenerationFeedbackData(m)).ToList())));
                    return false;
                }
